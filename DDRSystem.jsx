...
function ObsCard({ obs, allImages }) {
  ...
  const imgs = (obs.resolvedImages || obs.images || []);
    ...
    <div style={{ border: "1px solid #e7e5e4", borderRadius: 14, overflow: "hidden", marginBottom: 14, background: "#fff" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 14, color: "#1c1917", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{obs.area}</span>
          <SourceBadge src={obs.source || "both"} />
        </div>
          ...
          <div>
            ...
            {imgs.length > 0 ? (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {imgs.map((img, i) => <ImageThumb key={i} src={img.base64} page={img.page} />)}
              </div>
            ) : (
...
function UploadPage({ onProcess, apiKey, setApiKey, processing }) {
  const [iFile, setIFile] = useState(null);
  const [tFile, setTFile] = useState(null);
  const canRun = iFile && tFile && apiKey.trim() && !processing;

  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 30, color: "#1c1917", marginBottom: 4 }}>Upload Reports</div>
        <div style={{ fontSize: 13, color: "#78716c" }}>Upload both PDFs to run the full diagnostic pipeline</div>
      </div>

      <div style={{ background: "#fff", border: "1px solid #e7e5e4", borderRadius: 14, padding: 24, marginBottom: 20 }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: "#1c1917", marginBottom: 6 }}>🔑 Anthropic API Key</div>
        <div style={{ fontSize: 12, color: "#78716c", marginBottom: 10 }}>Stays in your browser · powers the AI analysis pipeline</div>
        <input
          type="password"
          value={apiKey}
          onChange={(e) => setApiKey(e.target.value)}
          placeholder="sk-ant-api03-…"
          style={{ width: "100%", padding: "10px 14px", border: "1px solid #e7e5e4", borderRadius: 10, fontFamily: "monospace", fontSize: 13, outline: "none", background: "#fafaf9" }}
        />
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        <div>
          <div style={{ fontWeight: 600, fontSize: 13, color: "#44403c", marginBottom: 8 }}>Inspection Report</div>
          <UploadZone label="Drop Inspection PDF" subtitle="Drag & drop or click to browse" file={iFile} onFile={setIFile} icon="🔍" />
        </div>
        <div>
          <div style={{ fontWeight: 600, fontSize: 13, color: "#44403c", marginBottom: 8 }}>Thermal Imaging Report</div>
          <UploadZone label="Drop Thermal PDF" subtitle="Drag & drop or click to browse" file={tFile} onFile={setTFile} icon="🌡" />
        </div>
      </div>
      ...
      <button
        ...
        style={{
          padding: "14px 32px", background: canRun ? "#1c1917" : "#d6d3d1",
          color: "#fff", border: "none", borderRadius: 99,
          fontWeight: 700, fontSize: 15, cursor: canRun ? "pointer" : "not-allowed",
      ...
      {!canRun && !processing && (
        <div style={{ marginTop: 10, fontSize: 12, color: "#a8a29e" }}>
          {!apiKey.trim() ? "Enter your API key" : !iFile ? "Upload the inspection report" : "Upload the thermal report"}
        </div>
      )}
    </div>
  );
...
function ReportPage({ report, onNav }) {
  ...
  const obsWithImgs = (ddr.area_observations || []).map((obs) => ({
    ...obs,
    resolvedImages: (allImages || []).filter((img) =>
      String(img.page) === String(obs.page_reference) ||
      (obs.source_ids || []).some((id) => imgMap[id])
    ),
  }));
        ...
        <>
          ...
          <section style={{ marginBottom: 36 }}>
            ...
            <ol style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {(ddr.recommended_actions || []).map((a, i) => (
...
export default function App() {
  ...
  const runPipeline = useCallback(async (iFile, tFile) => {
    ...
    try {
      ...
      log("Parsing inspection PDF…");
      const iParsed = await parsePDF(iFile);
      log(`Inspection: ${iParsed.numPages} pages, ${iParsed.images.length} images`);
      log("Parsing thermal PDF…");
      const tParsed = await parsePDF(tFile);
      log(`Thermal: ${tParsed.numPages} pages, ${tParsed.images.length} images`);
      setStep("parse", "done");

      // 2. Extract
      setStep("extract", "active");
      log("Extracting observations with AI…");
      const observations = await stepExtract(apiKey, iParsed.text, tParsed.text);
      log(`${observations.length} observations extracted`);
      setStep("extract", "done");

      // 3. Map images
      setStep("map", "active");
      log("Mapping images to observations…");
      const mapped = await stepMapImages(apiKey, observations,
        iParsed.images.map(i => ({...i, source:"inspection"})),
        tParsed.images.map(i => ({...i, source:"thermal"}))
      );
      setStep("map", "done");
      log("Image mapping complete");

      // 4. Reason
      setStep("reason", "active");
      log("Cross-document reasoning…");
      const merged = await stepReason(apiKey, mapped);
      log(`${merged.length} merged findings`);
      setStep("reason", "done");

      // 5. Validate
      setStep("validate", "active");
      log("Validating data…");
      const validation = await stepValidate(apiKey, merged);
      log(`${validation.conflicts?.length || 0} conflicts, ${validation.missing_info?.length || 0} gaps`);
      setStep("validate", "done");

      // 6. Generate DDR
      setStep("generate", "active");
      log("Generating DDR report…");
      const ddr = await stepGenerateDDR(apiKey, merged, validation);
      ...
      setReport({ ddr, validation, allImages, merged });
      setTimeout(() => { setPage("report"); setProcessing(false); }, 900);
    } catch (err) {
    ...
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div className="sidebar no-print" style={{
          width: 220, minWidth: 220, background: "#1c1917", color: "#fff",
          display: "flex", flexDirection: "column", padding: "28px 0",
          position: "sticky", top: 0, height: "100vh",
        }}>
          <div style={{ padding: "0 24px 24px", borderBottom: "1px solid rgba(255,255,255,.08)", marginBottom: 10 }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, color: "#fff", letterSpacing: -1 }}>DDR</div>
            <div style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(255,255,255,.3)", letterSpacing: 2.5, textTransform: "uppercase", marginTop: 2 }}>Diagnostic System</div>
          </div>

          {NAV.map((n) => (
            <button key={n.id} onClick={() => setPage(n.id)} style={{
              display: "flex", alignItems: "center", gap: 10,
              padding: "10px 24px", background: page === n.id ? "rgba(217,119,6,.2)" : "transparent",
              border: "none", borderLeft: page === n.id ? "3px solid #d97706" : "3px solid transparent",
              color: page === n.id ? "#fff" : "rgba(255,255,255,.5)",
              fontSize: 14, fontWeight: 600, cursor: "pointer", width: "100%", textAlign: "left",
              transition: "all .15s", fontFamily: "'Lato', sans-serif",
            }}>
              <span style={{ fontSize: 16 }}>{n.icon}</span> {n.label}
            </button>
          ))}

          <div style={{ marginTop: "auto", padding: "0 24px", fontSize: 10, color: "rgba(255,255,255,.22)", lineHeight: 1.8 }}>
            DDR System v2.0<br />
            Powered by Claude AI<br />
            {report && <span style={{ color: "rgba(255,255,255,.4)" }}>Report ready ✓</span>}
          </div>
        </div>

        {/* MAIN */}
        <div style={{ flex: 1, overflowY: "auto", background: "#f5f5f4" }}>
          <div style={{ maxWidth: 960, margin: "0 auto", padding: "40px 40px" }}>
            {page === "dashboard"  && <DashboardPage report={report} onNav={setPage} />}
            {page === "upload"     && <UploadPage onProcess={runPipeline} apiKey={apiKey} setApiKey={setApiKey} processing={processing} />}
            {page === "processing" && <ProcessingPage pipeStatus={pipeStatus} log={pipeLog} error={pipeError} />}
            {page === "report"     && <ReportPage report={report} onNav={setPage} />}
          </div>
        </div>
      </div>
    </>
